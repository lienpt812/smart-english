...
export function AnalyticsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabaseSelect<any>("sessions", { select: "kind,started_at,ended_at", order: "started_at.desc", limit: 100 }),
  ...
  const studyTime = useMemo(() => ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day, index) => ({
    day,
    minutes: sessions.reduce((sum, session) => {
      if (!session.started_at || !session.ended_at || new Date(session.started_at).getDay() !== index) return sum;
      return sum + Math.max(0, Math.round((new Date(session.ended_at).getTime() - new Date(session.started_at).getTime()) / 60000));
    }, 0),
  })), [sessions]);
  ...
  const heatData = Array.from({ length: 28 }, (_, i) => {
    ...
    const minutes = sessions.reduce((sum, session) => {
      if (!session.started_at || !session.ended_at) return sum;
      return new Date(session.started_at).toDateString() === date.toDateString()
        ? sum + Math.max(0, Math.round((new Date(session.ended_at).getTime() - new Date(session.started_at).getTime()) / 60000))
        : sum;
    }, 0);
  ...
  return (
    <div className="p-6 pb-24 lg:pb-6 max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-0.5" style={{ fontSize: "0.875rem" }}>Real learning metrics from Supabase</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Total Study Time", value: `${(totalMinutes / 60).toFixed(1)}h`, sub: "tracked", color: "#2D6A4F" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl border border-border p-5">
          <h3 className="text-foreground font-semibold mb-4" style={{ fontSize: "0.875rem" }}>Weekly Study Time</h3>
      ...
      <div className="bg-white rounded-2xl border border-border p-5">
        <h3 className="text-foreground font-semibold mb-4" style={{ fontSize: "0.875rem" }}>Last 28 Days</h3>
